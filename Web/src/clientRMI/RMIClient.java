				...
				while (!loggedIn){
					...
					if (username.equals("") && password.equals("")){
		                ...
		                if(stringSplitted.length == 4 && stringSplitted[0].equals("register")){
		                	...
		                	serverAnswer = server.clientRegister(username, password, mail, (ServerOperations) client, true);
		                	
		                } else if(stringSplitted.length == 3 && stringSplitted[0].equals("login")){
		                	username = stringSplitted[1];
		                	password = stringSplitted[2];
			                
		                	serverAnswer = (String) server.clientLogin(username, password, client, true);
		                }
					...
					/* The client is logged in already. */
						...
						serverAnswer = server.clientLogin(username, password, (ServerOperations) client, true);
						if (!serverAnswer.equals("log successful")){
			        		username = "";
			        		password = "";
